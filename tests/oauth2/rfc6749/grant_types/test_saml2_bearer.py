...
from oauthlib.oauth2.rfc6749.grant_types import SAML2BearerGrant
from oauthlib.oauth2.rfc6749.tokens import BearerToken

from ....unittest import TestCase


def extract_assertion(assertion, client, request):
    request.username = 'john'
    request.user = 'john'
    return True


class SAML2BearerGrantTest(TestCase):
    ...
    def test_create_token_response(self):
        self.mock_validator.validate_saml2_response.side_effect = extract_assertion
        ...
        self.mock_validator.validate_saml2_response.return_value = True
    ...
    def test_custom_auth_validators_unsupported(self):
        ...
        expected = ('SAML2BearerGrant does not '
                    'support authorization validators. Use token '
                              'validators instead.')
        with self.assertRaises(ValueError) as caught:
            SAML2BearerGrant(self.mock_validator, pre_auth=[authval1])
        self.assertEqual(caught.exception.args[0], expected)
        with self.assertRaises(ValueError) as caught:
            SAML2BearerGrant(self.mock_validator, post_auth=[authval2])
        self.assertEqual(caught.exception.args[0], expected)
